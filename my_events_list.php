...
$url = '/blocks/exaport/my_events_list.php';
...
block_exaport_print_header("events");
...
$owncats = $DB->get_records_select("block_exaportcate", "userid = 2", $conditions, "name ASC");
        ...
        foreach ($views as $viewBlock){
            echo '<div class="col-xl-4 col-md-6 col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">'.(strlen($viewBlock->name) > 60 ? mb_substr($viewBlock->name, 0, 60).'...' : $viewBlock->name).'</h5> 
                            <p class="card-text">'.htmlspecialchars(strlen(strip_tags($viewBlock->description)) > 30 ? mb_substr(strip_tags($viewBlock->description), 0, 30).'...' : strip_tags($view->description)).'</p>
                        </div>
                        <div class="card-body">
                            <a href="'.s($CFG->wwwroot.'/blocks/exaport/my_event_details.php?viewID='.$viewBlock->id.'').'"class="btn btn-primary">Details</a>
                        </div>
                    </div>
                </div>';
        }
