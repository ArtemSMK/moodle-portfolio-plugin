...
$query = "select b.*". // , i.*, i.id as itemid".
        ...
        " WHERE b.id = ? ORDER BY b.timemodified";     

        $query3 = " select b.* "." FROM {block_exaportviewachievement} b "." WHERE b.viewid = $view->id ";

$blocks = $DB->get_records_sql($query, array($view->id));
$blocks2 = $DB->get_record_sql($query2, array($view->id));
$connectionInfo = $DB->get_records_sql($query3);
$resumeArray = array();
$certifes = array();
$publics = array();

foreach($connectionInfo as $info){
if (!is_null($info->resume_id)){
    $resumesQuery = " select b.* FROM {block_exaportresume} b WHERE b.id = ? ";
    $valueRes =$DB->get_record_sql($resumesQuery, array($info->resume_id));
    array_push($resumeArray, $valueRes);
}
if (!is_null($info->certif_id)){
    $certifQuery = " select b.*FROM {block_exaportresume_certif} b WHERE b.id = ? ";
    $valueCert =$DB->get_record_sql($certifQuery, array($info->certif_id));
    array_push($certifes, $valueCert);
}
if (!is_null($info->public_id)){
    $publicQuery = " select b.*FROM {block_exaportresume_public} b WHERE b.id = ? ";
    $valuePub =$DB->get_record_sql($publicQuery, array($info->public_id));
    array_push($publics, $valuePub);
}
...
echo '<h3 class="h3 mb-3">Event details</h3>';
echo '<div class="row">';
echo "<div class='bold col-sm-3 mb-2'>Event name:</div>";
echo "<div class='col-sm-9 mb-2'>".$blocks2->name."</div>";
echo "<div class='bold col-sm-3'>Event description:</div>";
echo "<div class='col-sm-9'>".$blocks2->description."</div>";
echo "</div>";
// $userQuery = "SELECT u.*"."FROM {user} u"."WHERE u.id = 1";
// $attachmentUser = $DB->get_record("user", array("id=>'2'"));
// print_r($attachmentUser);
echo "<hr/>";
echo "<h3 class='h3 mb-3'>Participating students</h3>";


foreach($resumeArray as $attachment) {
    $attachmentUser = $DB->get_record("user", array('id'=>$attachment->user_id));
    echo '<div class="col-xl-4 col-md-6 col-sm-12">
            <div class="card" style =" width:700px">
                <div class="card-body">
                <h6 class="card-title">'.$attachmentUser->firstname." ".$attachmentUser->lastname.'</h6> 
                <p class="card-text">'.$attachment->cover.'</p> 
                </div>
            </div>
        </div>';
}

foreach($publics as $attachment) {
    $attachmentResume = $DB->get_record('block_exaportresume', array('id'=>$attachment->resume_id));
    $attachmentUser = $DB->get_record("user", array('id'=>$attachmentResume->user_id));
    echo '<div class="col-xl-4 col-md-6 col-sm-12">
    ...
    <span class="avatars" >
        ...
        <span class="userinitials size-35" style ="margin-top:20px;height:60px;width:60px">'.$attachmentUser->firstname[0].''.$attachmentUser->lastname[0].'</span>
        </span>
        </span>
        </div>
        <div class="col-md-8">
        ...
        <p class="card-text">'.$attachment->contributiondetails.'</p>
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalLong">Open details</button>
        </div>
        </div>
    </div>
</div>
...
foreach($certifes as $attachment) {
    $attachmentResume = $DB->get_record('block_exaportresume', array('id'=>$attachment->resume_id));
    $attachmentUser = $DB->get_record("user", array('id'=>$attachmentResume->user_id));
    echo '<div class="col-xl-4 col-md-6 col-sm-12">
            <div class="card mb-3" style =" width:700px">
...
// $ul_str.="</ul>";
// echo $ul_str;
$attachmentResume = $DB->get_record('block_exaportresume', array('id'=>$publics[0]->resume_id));
$attachmentUser = $DB->get_record("user", array('id'=>$attachmentResume->user_id));
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">'.$attachmentUser->firstname.' '.$attachmentUser->lastname.'</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <h6 class="card-text">Publication title:</h6><p>'.$publics[0]->title.'</p>
        <h6 class="card-text">Date:</h6><p>'.$publics[0]->date.'</p>
        <h6 class="card-text">Contribution:</h6><p>'.$publics[0]->contribution.'</p>
        <h6 class="card-text">Contribution Details:</h6><p>'.$publics[0]->contributiondetails.'</p>
        <h6 class="card-text">URL:</h6><a href='.$publics[0]->url.'>'.$publics[0]->url.'</a>
      </div>
